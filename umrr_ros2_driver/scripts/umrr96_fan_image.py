...
class FanImage:
    ...
    def __init__(self, max_range, half_angle, range_bin=.25, angle_bin_degrees=2.0,
        ...
        self.range_bins = math.ceil(max_range / range_bin)
        self.angle_bins = math.ceil(2 * half_angle / math.radians(angle_bin_degrees))
        if self.range_bins * self.angle_bins > 1000000:
    ...
    def _draw_guides(self):
        ...
        for i in range(6):
            self.text(canvas, str(i * 10), (60 + i * (self.width - 120) / 5,
                                          self.height - 35), centered=True)
